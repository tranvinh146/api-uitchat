...
channelSchema.statics.deleteChannel = async function (userId, serverId, channelId) {
    try {
        let currentServer = await Server.findById(serverId);
        let ownersServer = currentServer.ownerIds;
        if (!ownersServer.includes(userId)) {
...
channelSchema.statics.updateChannel = async function (userId, serverId, channelId, channelName) {
    try {
        let currentServer = await Server.findById(serverId);
        let ownersServer = currentServer.ownerIds;
        if (!ownersServer.includes(userId)) {
...
channelSchema.statics.deleteMembersByChannelId = async function (userId, serverId, channelId, memberIds) {
    try {
        let currentServer = await Server.findById(serverId);
        let ownersServer = currentServer.ownerIds;
        if (!ownersServer.includes(userId)) {
...
channelSchema.statics.updateMembersByChannelId = async function (userId, serverId, channelId, memberIds) {
    try {
        let currentServer = await Server.findById(serverId);
        let ownersServer = currentServer.ownerIds;
        if (!ownersServer.includes(userId)) {
...
channelSchema.statics.deleteOwnersByChannelId = async function (userId, serverId, channelId, ownerIds) {
    try {
        let currentServer = await Server.findById(serverId);
        let ownersServer = currentServer.ownerIds;
        if (!ownersServer.includes(userId)) {
...
channelSchema.statics.updateOwnersByChannelId = async function (userId, serverId, channelId, ownerIds) {
    try {
        let currentServer = await Server.findById(serverId);
        let ownersServer = currentServer.ownerIds;
        if (!ownersServer.includes(userId)) {
