...
ConversationSchema.statics.getConversationByUserId = async function (
  ownerId,
  conversationPerPage = 10
) {
  try {
    const conversationsList = await this.find({
      ConversationCode: { $regex: ownerId },
    }).sort({ updatedAt: -1 });
    ...
    const totalNumConversations = await this.count({
      ConversationCode: { $regex: ownerId },
    });
...
ConversationSchema.statics.addNewConversation = async function (
  ownerId,
  memberId
) {
  try {
    ...
    let conversation = await this.findOne({
      $or: [{ ConversationCode: senderId }, { ConversationCode: receiverId }],
    });
    ...
    if (!conversation) {
      ...
      return {
        status: "Created success conversation",
        conversation: conversation,
      };
    } else {
      return {
        status: "The conversation has existed",
        conversation: conversation,
      };
    }
  } catch (e) {
